...
pub fn run(sourcemap: String, packages_folder: String) -> Result<()> {
    let sourcemap_contents =
        std::fs::read_to_string(sourcemap).context("Failed to read sourcemap file")?;
    ...
    let mut success = true;
    for entry in std::fs::read_dir(packages_folder)
        ...
        .flatten()
    {
        if let Ok(file_type) = entry.file_type() {
            ...
            bail!("Couldn't get file type for {:?}", entry.path());
        }
    }
